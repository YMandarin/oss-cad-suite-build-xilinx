...
if (-not $env:YOSYSHQ_ROOT) {
    $env:YOSYSHQ_ROOT = (Get-Item -LiteralPath $PSScriptRoot).FullName
}
...
$env:PATH = @((Join-Path $env:YOSYSHQ_ROOT "bin"), (Join-Path $env:YOSYSHQ_ROOT "lib"), $env:PATH) -join ";"
...
& (Join-Path $env:YOSYSHQ_ROOT "lib\gdk-pixbuf-query-loaders.exe") --update-cache
